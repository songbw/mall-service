...
public class AdminOrderServiceImpl implements AdminOrderService {
    ...
    /**
     * 导出订单对账单 - 获取导出的vo : List<ExportOrdersVo>
     *
     * 1.获取"已完成","已退款"状态的子订单
     * 2.拼装导出数据
     *
     ...
     * @return
     */
    public List<ExportOrdersVo> exportOrdersReconciliation(OrderExportReqVo orderExportReqVo) throws Exception {
        // 1. 获取"已完成","已退款"状态的子订单
        // 查询条件转数据库实体
        OrderDetail orderDetail = convertToOrderDetailForReconciliation(orderExportReqVo);
        log.info("导出订单对账单 根据条件获取子订单集合 查询数据库条件:{}", JSONUtil.toJsonString(orderDetail));
        ...
        List<Orders> ordersList = adminOrderDao.selectExportOrders(null, startDateTime, endDateTime);
        log.info("导出订单 查询数据库结果List<Orders>:{}", JSONUtil.toJsonString(ordersList));
     ...
     * @return
     */
    private OrderDetail convertToOrderDetailForReconciliation(OrderExportReqVo orderExportReqVo) {
        OrderDetail orderDetail = new OrderDetail();
        orderDetail.setMerchantId(orderExportReqVo.getMerchantId());
        return orderDetail;
    }
