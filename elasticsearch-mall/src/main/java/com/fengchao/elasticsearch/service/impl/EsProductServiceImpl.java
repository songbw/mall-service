...
import com.fengchao.elasticsearch.repository.EsProductRepository;
...
public class EsProductServiceImpl implements EsProductService {
    ...
    public int importAll() {
        OperaResult operaResult = productFeign.getAllEsProductList();
        Map<String, Object> map = operaResult.getData();
        List<AoyiProdIndex> aoyiProdIndices = (List<AoyiProdIndex>) map.get("result");
        ObjectMapper objectMapper = new ObjectMapper();
        List<EsProduct> esProductList = new ArrayList();
        aoyiProdIndices.forEach(aoyiProdIndex -> {
            EsProduct product = new EsProduct();
            BeanUtils.copyProperties(aoyiProdIndex, product);
    ...
    public EsProduct create(Long id) {
        ...
        List<EsProduct> esProductList = null;
//        productDao.getAllEsProductList(id);
...
//        productDao.getAllEsProductList(id);
        if (esProductList.size() > 0) {
            ...
            String productCategoryId = esProduct.getCategoryId();
            //根据商品标题、品牌、分类进行搜索
            List<FunctionScoreQueryBuilder.FilterFunctionBuilder> filterFunctionBuilders = new ArrayList<>();
            filterFunctionBuilders.add(new FunctionScoreQueryBuilder.FilterFunctionBuilder(QueryBuilders.matchQuery("name", keyword),
